...
export const themVaoGioHang = async ({ mataikhoan, masanpham, soluong }) => {
  const response = await axios.post(`${API_BASE}/add`, { mataikhoan, masanpham, soluong }, { withCredentials: true });
  return response.data;
};

// 📋 Lấy toàn bộ sản phẩm trong giỏ hàng của một tài khoản
export const getGioHangByTaiKhoan = async (mataikhoan) => {
  const response = await axios.get(`${API_BASE}/user/${mataikhoan}`, { withCredentials: true });
...
export const capNhatSoLuong = async ({ magiohang, soluong }) => {
  const response = await axios.put(`${API_BASE}/update`, { magiohang, soluong }, { withCredentials: true });
  return response.data;
};

// ❌ Xoá sản phẩm khỏi giỏ hàng
export const xoaKhoiGioHang = async (magiohang) => {
  const response = await axios.delete(`${API_BASE}/${magiohang}`, { withCredentials: true });
  return response.data;
};
